...
export default class eventsController {
    ...
    static async createEvent(req, res) {
        try {
            ...
            const eventId = await eventsTable.create(name, content, start, end, type, color ? color : "#fff")
    ...
    static async updateEvent(req, res) {
        try {
            ...
            const {calendarId, newName, newContent, newStart, newEnd, newType, newColor}  = req.body;
            ...
            if(newName)
                await eventsTable.update(eventId, "name", newName);

            if(newContent)
                await eventsTable.update(eventId, "content", newContent);

            if(newStart)
                await eventsTable.update(eventId, "start", newStart);

            if(newEnd)
                await eventsTable.update(eventId, "end", newEnd);

            if(newType)
                await eventsTable.update(eventId, "type",  newType);

            if(newColor)
                await eventsTable.update(eventId, "color", newColor);
